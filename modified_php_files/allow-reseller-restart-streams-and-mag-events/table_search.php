    ...
    if ($rReturn["recordsTotal"] > 0) {
        ...
        if (($rResult) && ($rResult->num_rows > 0)) {
            while ($rRow = $rResult->fetch_assoc()) {
                ...
                // send uptime and restart buttons for allowed reseller groups with next 2 lines.  
                } else if ($rPermissions["reseller_controls_streams"]) {
                    $rReturn["data"][] = Array($rRow["id"], $rIcon, $rStreamName, $rServerName, $rUptime, $rButtons, $rStreamInfoText);
                } else {

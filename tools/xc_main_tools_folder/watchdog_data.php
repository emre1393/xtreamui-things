...
while (!false) {
    ...
    $wd_data = array_merge(D6E530a9573198395BDb5822B82478e2(),$jsonOutput); 
    if (!$f566700a43ee8e1f0412fe10fbdf03df->query('UPDATE `streaming_servers` SET `watchdog_data` = \'%s\' WHERE `id` = \'%d\'', json_encode($wd_data, JSON_PARTIAL_OUTPUT_ON_ERROR), SERVER_ID)) {
        break;
    }
    sleep(2);
}

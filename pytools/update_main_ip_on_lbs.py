    ...
    for rServer in rServers:
        if int(rServer[0]) == int(rConfig["server_id"]):
            ...
            if rRDB:
                ...
                while rQ not in ["Y", "N"]: rQ = raw_input("Would you like to update main server ip? (Enter Y/N) : ").upper()
                rNewMainIP = False
                if rQ == "Y":
                    rNewMainIP = True
                else:
                ...
                try: new_main_ip = raw_input("New Main Server Ip: ")
                except:
                    ...
                    if int(rServer[0]) <> int(rConfig["server_id"]):
                        if rNewMainIP:
                            ...
                            try: rOldConfig = decryptConfig(requests.post(rAPI, data=rData, timeout=5).content)
                                ...
                                rOldConfig = {'server_id': str(rServer[0]), 'db_name': rConfig["db_name"], 'pconnect': '0', 'db_port': str(rConfig["db_port"]), 'db_pass': rConfig["db_pass"], u'host': None, 'db_user': rConfig["db_user"]}
